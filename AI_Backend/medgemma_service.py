"""
MedGemma Service — Modal Proxy
================================
Handles all interactions with the MedGemma-27B model hosted on Modal.
Provides:
  1. NER extraction from dialogue transcripts
  2. Clinical narrative generation (SOAP)
  3. EMR record building
  4. Patient-friendly summary generation
  5. Differential diagnosis suggestions
  6. Generic chat completions

All inference runs on Modal — this service only formats prompts and
parses responses.
"""
...
from schemas import (
    ...
    Procedure, FamilyHistoryItem,
    EMRRecord, PatientDemographics, EncounterInfo, EncounterType,
    EncounterStatus, ClinicalNarratives, ReviewOfSystems,
...
class MedGemmaService:
    """
    Proxy to MedGemma-27B on Modal. Provides NER extraction,
    narrative generation, EMR building, and patient summary.
    """

    def __init__(self, modal_url: str):
        self.base_url = modal_url.rstrip("/")
        # Strip full endpoint path if provided
        if self.base_url.endswith("/v1/chat/completions"):
            self.base_url = self.base_url[:-20]
        self._client: httpx.AsyncClient = None
        self._icd_service = None
        self.loaded = False
        self._ready = False  # True only when URL is configured and client is created

    def set_icd_service(self, icd_service):
        """Attach ICD service for auto-mapping codes to entities."""
        self._icd_service = icd_service

    async def load(self):
        """Initialize the HTTP client and verify connectivity."""
        if not self.base_url:
            logger.warning("MODAL_MEDGEMMA_URL not set — MedGemma service will be unavailable")
            return
        ...
        try:
            ...
            if resp.status_code == 200:
                logger.info(f"MedGemma Modal endpoint ready: {self.base_url}")
            else:
                logger.info(f"MedGemma endpoint configured: {self.base_url} (status {resp.status_code})")
        except Exception as e:
            logger.warning(f"MedGemma connectivity check failed (may still work): {e}")

        self.loaded = True
        self._ready = True

    async def generate(self, prompt: str, max_new_tokens: int = 1500) -> str:
        """Call MedGemma-27B on Modal via OpenAI-compatible chat completions."""
        if not self._client:
            raise RuntimeError("MedGemma service not loaded. Call load() first.")
        ...
        for attempt in range(1, 4):
            try:
                ...
                wait = 2 ** attempt
                logger.warning(
                    f"MedGemma call failed (attempt {attempt}/3): {type(e).__name__}: {e}. "
                ...
                try:
                    ...
                    pass
                self._client = httpx.AsyncClient(
                    base_url=self.base_url, timeout=300.0, follow_redirects=True,
                )
    ...
    async def chat_completion(
        ...
        """Proxy a generic chat completion request to MedGemma. Returns the text content."""
        if not self._client:
            raise RuntimeError("MedGemma service not loaded.")
    ...
    async def extract_entities(self, transcript_text: str) -> ExtractedEntities:
        """Extract structured medical entities from a transcript string."""
        prompt = NER_PROMPT.format(dialogue=transcript_text)
        raw = await self.generate(prompt, max_new_tokens=1500)
    ...
    def _enrich_with_icd(self, entities: ExtractedEntities) -> ExtractedEntities:
        """Attach ICD-10 codes to symptoms and diagnoses."""
    ...
    async def generate_narratives(
        ...
        # Truncate transcript to first ~500 chars for the summary
        dialogue_summary = transcript_text[:800] if transcript_text else ""

        prompt = NARRATIVE_PROMPT.format(
            entities=entities_json,
            dialogue_summary=dialogue_summary,
        )
        raw = await self.generate(prompt, max_new_tokens=800)
    ...
    async def build_emr(
        ...
        narr_data = await self.generate_narratives(entities, transcript_text)

        # Build ReviewOfSystems
        ...
        # Normalize encounter_date to ISO 8601 (YYYY-MM-DD) — FHIR R4 requires this.
        # MedGemma or the caller may supply DD-MM-YYYY or DD/MM/YYYY formats.
        def _to_iso_date(raw: Optional[str]) -> str:
            ...
            for fmt in ("%d-%m-%Y", "%d/%m/%Y", "%Y-%m-%d", "%m-%d-%Y", "%m/%d/%Y"):
                try:
                    ...
                    continue
            # Return as-is if already parseable or unknown format
            return raw

        enc_date = _to_iso_date(encounter_date)
        ...
        # Collect all ICD codes, deduplicate
        all_icd = []
        ...
        # Separate surgical history
        surgical_keywords = {
            "surgery", "surgical", "appendectomy", "cholecystectomy",
    ...
    async def generate_patient_summary(self, emr_json: str) -> str:
        """Generate a patient-friendly summary from EMR JSON."""
...
def _parse_json(raw: str) -> dict:
    ...
    # Strategy 4: Truncated JSON — close open braces
    last_brace = cleaned.rfind('{')
    if last_brace >= 0:
        truncated = cleaned[last_brace:]
        open_count = truncated.count('{') - truncated.count('}')
        if open_count > 0:
            repaired = truncated + ']}' * 2 + '}' * open_count
            try:
